...
class MancalaBoard:
    ...
    def legalMoves( self, player ):
        ...
        for m in range(len(cups)):
            if cups[m] != 0:
                moves += [m+1] #we can move to m+1 from m cup
        return moves
    ...
    def makeMove( self, player, cup ):
        ...
        if self.gameOver():
            # clear out the cups and move them to respective mancala if the game is over
            for i in range(len(self.P1Cups)):
                self.scoreCups[0] += self.P1Cups[i]
                self.P1Cups[i] = 0  #making the chosen cup with zero value
    ...
    def makeMoveHelp( self, player, cup ):
        ...
        cup += 1  #to give stone from the next cup
    ...
    def hasWon( self, playerNum ):
        ...
        if self.gameOver():
            opp = 2 - playerNum + 1
            return self.scoreCups[playerNum-1] > self.scoreCups[opp-1]#return true if given player has greater score
        else:
            return False
        ...
        while not(self.gameOver()):
            ...
            while again:
                ...
                while not(self.legalMove(currPlayer, move)):
                    ...
                    move = currPlayer.chooseMove(self) #if the move is illegal then player has to choose move again
